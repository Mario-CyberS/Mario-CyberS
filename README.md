...
I’m a Cyber Security enthusiast with hands-on experience in SIEM deployment/configuration, scripting, automation, threat log analysis, and network/endpoint hardening. As a Cyber Security Analyst Intern at Diverse Computing, Inc., I deployed and tuned Wazuh SIEMs on RHEL/Almalinux servers for centralized log aggregation and threat detection. I rolled out Wazuh agents across endpoints for monitoring (FIM, configuration assessment, and threat hunting), automated ASA firewall backups with custom Expect/Bash scripts, and supported the Networking team in investigating unauthorized changes. I also leveraged tools like Zabbix for monitoring and automated disruption alerting.

I also maintain a homelab where I build secure networking environments, using a Raspberry Pi as a firewall/NAT relay, Tailscale for private VPN access, and Mullvad as a traffic exit node. A key project is the deployment of an enterprise-style Wazuh SIEM environment built on AlmaLinux. The project includes centralized log collection, custom decoders and detection rules, automated Cisco ASA configuration backups, secure Postfix email alerting, and endpoint monitoring. Additional work included integrating rsyslog, creating custom Python automations, implementing configuration retention policies, and documenting the deployment end-to-end. This project demonstrates practical experience with SIEM engineering, log management, Linux administration, detection engineering, automation, and security operations.
...
#### [Phase 2 — Workstation Join + Identity](https://github.com/Mario-CyberS/Phase-2-Workstation-Join-Identity/blob/main/README.md)
