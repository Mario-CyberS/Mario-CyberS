...
I also maintain a homelab where I build secure networking environments, using a Raspberry Pi as a firewall/NAT relay, Tailscale for private VPN access, and Mullvad as a traffic exit node. A key project is the Automated Cyberscan Orchestrator, developed with a collaborator to automate bug bounty reconnaissance. It leverages Python/Selenium with undetected-chromedriver to scrape bounty targets, routes findings through an LLM for context-aware tool selection, and automates recon tool execution with organized reporting. Future plans include a CMP server to enable scalable, orchestrated vulnerability workflows.
